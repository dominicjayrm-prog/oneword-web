...
interface LocaleContent {
  scene1: { tagline: string; line1a: string; line1b: string; line1c: string; line2a: string; line2b: string; subtitle: string };
  scene2: { label: string; word: string; category: string; bgWords: string[] };
  phone: {
    streakLabel: string; todaysWord: string; word: string; categoryLabel: string;
    describeLabel: string; typingWords: string[]; submitLabel: string; lockedLabel: string;
    whichBetter: string; desc1: string; desc2: string; pickLabel: string; voteLabel: string;
    rankLabel: string; outOf: string; topLabel: string; shareLabel: string;
    entries: { e: string; t: string; u: string; v: number }[];
    descCount: string;
  };
  share: { label: string; wordLabel: string; desc: string; stats: { icon: string; value: string; label: string }[]; cta: string };
  final: { subtitle: string; desc: string; platforms: string; ctaLabel: string; joinLabel: string };
  overlay: { subtitle: string; tapLabel: string };
}

const EN: LocaleContent = {
  scene1: { tagline: "every single day", line1a: "The ", line1b: "whole", line1c: " world.", line2a: "One ", line2b: "word.", subtitle: "Say it in five." },
  scene2: { label: "today\u2019s word is", word: "SILENCE", category: "abstract", bgWords: ["RAIN", "LOVE", "FEAR", "PIZZA", "HOME", "WIFI"] },
  phone: {
    streakLabel: "12 day streak", todaysWord: "today\u2019s word", word: "SILENCE", categoryLabel: "abstract \u00b7 day 3",
    describeLabel: "describe it in exactly 5 words", typingWords: ["Loudest", "sound", "nobody", "can", "hear"],
    submitLabel: "SUBMIT", lockedLabel: "\u2713 LOCKED IN", whichBetter: "which is better?",
    desc1: "Loudest sound nobody can hear", desc2: "When words take their break", pickLabel: "YOUR PICK \u2713",
    voteLabel: "vote 7 of 15", rankLabel: "you ranked", outOf: "out of 3,847", topLabel: "Top descriptions",
    shareLabel: "\ud83d\udce4 Share Results",
    entries: [
      { e: "\ud83e\udd47", t: "The truth behind everything happening", u: "@stevie", v: 847 },
      { e: "\ud83e\udd48", t: "Peace wearing invisible costume today", u: "@sara", v: 723 },
      { e: "\ud83e\udd49", t: "What guilty people fear most", u: "@luna", v: 694 },
    ],
    descCount: "3,847 descriptions",
  },
  share: { label: "share your result", wordLabel: "today\u2019s word", desc: "Loudest sound nobody can hear", stats: [{ icon: "\ud83d\udcca", value: "#7", label: "RANK" }, { icon: "\u2764\ufe0f", value: "694", label: "VOTES" }, { icon: "\ud83d\udd25", value: "12", label: "STREAK" }], cta: "How would you describe it?" },
  final: { subtitle: "Say it in five.", desc: "The daily game for creative minds.", platforms: "iOS \u00b7 Android \u00b7 Web", ctaLabel: "Play Now \u2014 Free", joinLabel: "Join thousands of daily players" },
  overlay: { subtitle: "Say it in five.", tapLabel: "Tap to play \u00b7 33s" },
};

const ES: LocaleContent = {
  scene1: { tagline: "cada d\u00eda del a\u00f1o", line1a: "El ", line1b: "mundo", line1c: " entero.", line2a: "Una ", line2b: "palabra.", subtitle: "Dilo en cinco." },
  scene2: { label: "la palabra de hoy es", word: "LUNES", category: "vida", bgWords: ["LLUVIA", "AMOR", "MIEDO", "PIZZA", "HOGAR", "WIFI"] },
  phone: {
    streakLabel: "racha de 12 d\u00edas", todaysWord: "la palabra de hoy", word: "LUNES", categoryLabel: "vida \u00b7 d\u00eda 2",
    describeLabel: "descr\u00edbela en exactamente 5 palabras", typingWords: ["Funeral", "semanal", "al", "que", "sobrevivimos"],
    submitLabel: "ENVIAR", lockedLabel: "\u2713 REGISTRADO", whichBetter: "\u00bfcu\u00e1l es mejor?",
    desc1: "Funeral semanal al que sobrevivimos", desc2: "El despertador celebra su venganza", pickLabel: "TU ELECCI\u00d3N \u2713",
    voteLabel: "voto 7 de 15", rankLabel: "tu puesto", outOf: "de 3.847", topLabel: "Mejores descripciones",
    ...
    descCount: "3.847 descripciones",
  },
  share: { label: "comparte tu resultado", wordLabel: "la palabra de hoy", desc: "Funeral semanal al que sobrevivimos", stats: [{ icon: "\ud83d\udcca", value: "#7", label: "PUESTO" }, { icon: "\u2764\ufe0f", value: "694", label: "VOTOS" }, { icon: "\ud83d\udd25", value: "12", label: "RACHA" }], cta: "\u00bfC\u00f3mo lo describir\u00edas t\u00fa?" },
  final: { subtitle: "Dilo en cinco.", desc: "El juego diario para mentes creativas.", platforms: "iOS \u00b7 Android \u00b7 Web", ctaLabel: "Juega Ya \u2014 Gratis", joinLabel: "\u00danete a miles de jugadores diarios" },
  overlay: { subtitle: "Dilo en cinco.", tapLabel: "Toca para ver \u00b7 33s" },
};
...
function useTimer() {
  ...
  const at = useCallback((t: number) => ms >= t, [ms]);
  const bw = useCallback((a: number, b: number) => ms >= a && ms < b, [ms]);
  const p = useCallback((a: number, b: number) => Math.min(1, Math.max(0, (ms - a) / (b - a))), [ms]);

  return { ms, at, bw, p, on, setOn };
}
...
function PhoneContent({ c: content, at, bw, fi, p }: {
  c: LocaleContent['phone'];
  at: (t: number) => boolean;
  bw: (a: number, b: number) => boolean;
  fi: (s: number, d?: number) => number;
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, justifyContent: "center", padding: "0 2px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "4px", padding: "3px 10px", borderRadius: "12px", backgroundColor: `${C.primary}10`, opacity: fi(6500) }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "8px", fontWeight: 600, color: C.primary }}>{content.streakLabel}</span>
        </div>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted, letterSpacing: "2.5px", textTransform: "uppercase", marginTop: "12px", opacity: fi(6800) }}>{content.todaysWord}</span>
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "40px", fontWeight: 700, color: C.text, margin: "2px 0", letterSpacing: "-1.5px", opacity: fi(7000, 700), transform: `scale(${L(0.9, 1, easeOut(p(7000, 7700)))})` }}>{content.word}</h2>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted, opacity: fi(7500) }}>{content.categoryLabel}</span>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted, letterSpacing: "2px", textTransform: "uppercase", marginTop: "16px", opacity: fi(8200) }}>{content.describeLabel}</p>
        ...
        <div style={{ marginTop: "8px", padding: "8px 20px", borderRadius: "11px", backgroundColor: at(13400) ? C.text : "#D1CCC0", width: "100%", textAlign: "center", transform: at(13800) ? "scale(1.03)" : "scale(1)", transition: "all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "10px", fontWeight: 700, color: "#FFF" }}>{at(13800) ? content.lockedLabel : content.submitLabel}</span>
        </div>
      </div>
    );
  }
  ...
  if (bw(14200, 18700)) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, justifyContent: "center", padding: "0 2px" }}>
        <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeight: 700, color: C.text, opacity: fi(14800) }}>{content.word}</span>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted, letterSpacing: "2px", textTransform: "uppercase", margin: "3px 0 14px", opacity: fi(15000) }}>{content.whichBetter}</span>
        <div style={{ padding: "14px 11px", borderRadius: "13px", width: "100%", backgroundColor: picked ? `${C.primary}10` : "#FFF", border: `2px solid ${picked ? C.primary : C.border}`, marginBottom: "8px", position: "relative", transform: `scale(${picked ? 1.03 : 1})`, opacity: fi(15300, 400), transition: "background-color 0.5s, border-color 0.5s, transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "11px", fontWeight: 500, color: C.text, margin: 0, fontStyle: "italic", textAlign: "center" }}>&quot;{content.desc1}&quot;</p>
          {picked && (<div style={{ position: "absolute", top: "-7px", right: "-3px", backgroundColor: C.primary, color: "white", borderRadius: "8px", padding: "2px 7px", fontSize: "7px", fontWeight: 700, fontFamily: "'DM Sans', sans-serif", opacity: fi(16800, 300), transform: `scale(${L(0.5, 1, spring(p(16800, 17200)))})` }}>{content.pickLabel}</div>)}
        </div>
        ...
        <div style={{ padding: "14px 11px", borderRadius: "13px", width: "100%", backgroundColor: "#FFF", border: `1.5px solid ${C.border}`, marginTop: "8px", opacity: picked ? 0.25 : fi(15700, 400), transform: `scale(${picked ? 0.96 : 1})`, transition: "opacity 0.5s, transform 0.5s" }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "11px", fontWeight: 500, color: C.text, margin: 0, fontStyle: "italic", textAlign: "center" }}>&quot;{content.desc2}&quot;</p>
        </div>
        <div style={{ marginTop: "10px", display: "flex", alignItems: "center", gap: "5px", opacity: fi(15500) }}>
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "8px", color: C.muted }}>{content.voteLabel}</span>
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: "0 2px" }}>
        <div style={{ textAlign: "center", marginBottom: "6px" }}>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "16px", fontWeight: 700, color: C.text, opacity: fi(18600) }}>{content.word}</span>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: "7px", color: C.muted, margin: "1px 0 0", opacity: fi(18800) }}>{content.descCount}</p>
        </div>
        <div style={{ padding: "8px 9px", borderRadius: "11px", backgroundColor: `${C.primary}08`, border: `1.5px solid ${C.primary}`, textAlign: "center", marginBottom: "8px", opacity: fi(19200, 600), transform: `scale(${L(0.85, 1, spring(p(19200, 19800)))})` }}>
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "6px", color: C.muted, textTransform: "uppercase", letterSpacing: "1px" }}>{content.rankLabel}</span>
          ...
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted }}>{content.outOf}</span>
        </div>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "7px", color: C.muted, textTransform: "uppercase", letterSpacing: "1px", marginBottom: "4px", fontWeight: 600, opacity: fi(20200) }}>{content.topLabel}</span>
        ...
        <div style={{ marginTop: "8px", padding: "8px", borderRadius: "11px", backgroundColor: C.primary, textAlign: "center", opacity: fi(21300), transform: `scale(${L(0.9, 1, spring(p(21300, 21700)))})` }}>
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "9px", fontWeight: 700, color: "#FFF" }}>{content.shareLabel}</span>
        </div>
      </div>
    );
  }
...
export default function PromoVideo({ locale = 'en' }: { locale?: string }) {
  const { ms, at, bw, p, on, setOn } = useTimer();
  ...
  const { scene1, scene2, share, final: fin, overlay } = content;
    ...
    >
      ...
      {bw(0, 3800) && (
        <div style={{ position: "absolute", inset: 0, zIndex: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 30px", textAlign: "center", opacity: bw(2800, 3800) ? fo(2800, 700) : 1 }}>
          <p style={{ fontFamily: "'DM Sans'", fontSize: "12px", color: C.mutedLight, letterSpacing: "5px", textTransform: "uppercase", opacity: fi(200, 600), transform: `translateY(${su(200, 600, 25)}px)`, margin: "0 0 16px" }}>{scene1.tagline}</p>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: locale === 'es' ? "48px" : "52px", fontWeight: 900, letterSpacing: "-2px", margin: "0", lineHeight: 1.05, opacity: fi(700, 800), transform: `translateY(${su(700, 800, 30)}px)` }}>
            <span style={{ color: C.white }}>{scene1.line1a}</span>
            <span style={{ color: C.white, textShadow: "0 0 30px rgba(255,255,255,0.25), 0 0 60px rgba(255,255,255,0.1)" }}>{scene1.line1b}</span>
            <span style={{ color: C.white }}>{scene1.line1c}</span>
          </h1>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: locale === 'es' ? "40px" : "44px", fontWeight: 900, letterSpacing: "-2px", margin: "2px 0 0", lineHeight: 1.05, opacity: fi(1400, 700), transform: `translateY(${su(1400, 700, 25)}px)` }}>
            <span style={{ color: C.white }}>{scene1.line2a}</span>
            <span style={{ color: C.primary, textShadow: `0 0 30px ${C.glow}` }}>{scene1.line2b}</span>
          </h2>
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeight: 500, fontStyle: "italic", color: C.primary, margin: "16px 0 0", letterSpacing: "0.5px", opacity: fi(2100, 500) }}>{scene1.subtitle}</p>
        </div>
      )}
      ...
      {bw(3000, 6300) && (
        <div style={{ position: "absolute", inset: 0, zIndex: bw(3000, 3800) ? 15 : 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: bw(3000, 3800) ? fi(3000, 800) : bw(5500, 6300) ? fo(5500, 600) : 1 }}>
          ...
          <span style={{ fontFamily: "'DM Sans'", fontSize: "11px", color: C.mutedLight, letterSpacing: "4px", textTransform: "uppercase", opacity: fi(3600, 400) }}>{scene2.label}</span>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: `${L(24, 72, ease(p(3900, 4800)))}px`, fontWeight: 900, color: C.white, letterSpacing: "-3px", margin: "6px 0 0", lineHeight: 1, textShadow: `0 0 40px ${C.glow}, 0 0 80px ${C.glowSoft}`, opacity: fi(3900, 600) }}>{scene2.word}</h1>
          <p style={{ fontFamily: "'DM Sans'", fontSize: "12px", color: C.primary, fontWeight: 600, letterSpacing: "3px", textTransform: "uppercase", margin: "10px 0 0", opacity: fi(5000, 400) }}>{scene2.category}</p>
        </div>
      )}
          ...
          <div style={{ transform: `scale(${globalScale()}) translateY(${globalY()}px)`, transition: "none" }}>
            <div style={{ width: "220px", height: "440px", borderRadius: "30px", backgroundColor: C.cream, border: "3px solid #2a2a3a", overflow: "hidden", position: "relative", boxShadow: `0 20px 80px rgba(0,0,0,0.5), 0 0 ${at(13800) ? "60" : "30"}px ${C.glow}`, transition: "box-shadow 0.6s ease" }}>
              ...
              <div style={{ padding: "28px 14px 14px", height: "100%", display: "flex", flexDirection: "column" }}>
                <PhoneContent c={content.phone} at={at} bw={bw} fi={fi} p={p} />
              </div>
            </div>
          </div>
        </div>
      ...
      {bw(22000, 26500) && (
        <div style={{ position: "absolute", inset: 0, zIndex: bw(22000, 22700) ? 15 : 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px", opacity: bw(22000, 22700) ? fi(22000, 700) : bw(25500, 26500) ? fo(25500, 700) : 1 }}>
          <p style={{ fontFamily: "'DM Sans'", fontSize: "11px", color: C.mutedLight, letterSpacing: "3px", textTransform: "uppercase", margin: "0 0 14px", opacity: fi(22500, 400) }}>{share.label}</p>
          <div style={{ width: "100%", maxWidth: "300px", padding: "24px 20px", borderRadius: "22px", background: `linear-gradient(145deg, ${C.dark1} 0%, ${C.dark2} 100%)`, textAlign: "center", boxShadow: `0 25px 80px rgba(0,0,0,0.6), 0 0 60px ${C.glow}`, opacity: fi(22800, 600), transform: `translateY(${su(22800, 700, 30)}px) scale(${L(0.92, 1, easeOut(p(22800, 23500)))})` }}>
            ...
            <span style={{ fontFamily: "'DM Sans'", fontSize: "7px", color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "2px" }}>{share.wordLabel}</span>
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "36px", fontWeight: 700, color: "#FFF", margin: "2px 0 12px", letterSpacing: "-1.5px" }}>{scene2.word}</h3>
            <div style={{ padding: "10px 12px", borderRadius: "12px", backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", marginBottom: "14px" }}>
              <p style={{ fontFamily: "'DM Sans'", fontSize: "14px", fontWeight: 500, color: "#FFF", margin: 0, fontStyle: "italic" }}>&quot;{share.desc}&quot;</p>
            </div>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "14px", fontStyle: "italic", color: C.primary, opacity: 0.7, marginTop: "16px" }}>{share.cta}</p>
        </div>
      )}
      ...
      {at(26000) && (
        <div style={{ position: "absolute", inset: 0, zIndex: 25, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 30px", opacity: fi(26000, 800) }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: "22px", fontStyle: "italic", color: C.primary, margin: "0 0 6px", textAlign: "center", letterSpacing: "0.5px", opacity: fi(27500, 500) }}>{fin.subtitle}</p>
          <p style={{ fontFamily: "'DM Sans'", fontSize: "13px", color: "rgba(255,255,255,0.3)", margin: "0 0 4px", textAlign: "center", opacity: fi(27800, 500) }}>{fin.desc}</p>
          <p style={{ fontFamily: "'DM Sans'", fontSize: "11px", color: "rgba(255,255,255,0.2)", margin: "0 0 28px", opacity: fi(28000, 500) }}>{fin.platforms}</p>
          <div style={{ padding: "16px 44px", borderRadius: "16px", backgroundColor: C.primary, boxShadow: `0 8px 40px ${C.glow}`, opacity: fi(28500, 600), transform: `scale(${L(0.85, 1, spring(p(28500, 29200)))})` }}>
            <span style={{ fontFamily: "'DM Sans'", fontSize: "18px", fontWeight: 700, color: "#FFF" }}>{fin.ctaLabel}</span>
          </div>
          <p style={{ fontFamily: "'DM Mono'", fontSize: "10px", color: "rgba(255,255,255,0.15)", marginTop: "14px", opacity: fi(29500, 500) }}>{fin.joinLabel}</p>
